...
public class CollectionManager {
    ...
    public void fillFromDB() {
        try (Connection connection = DBUtils.getConnection()){
            ...
            while (rs.next()) {
                collection.add(new Movie(rs.getLong(1), rs.getString(2), new Coordinates(rs.getInt(3), rs.getInt(4)), LocalDateTime.parse(rs.getString(5)), rs.getLong(6), rs.getString(7) == null ? null : MovieGenre.valueOf(rs.getString(7)), MpaaRating.valueOf(rs.getString(8)), new Person(rs.getString(9), rs.getString(10) == null ? null : LocalDate.parse(rs.getString(10)), rs.getDouble(11), new Location(rs.getInt(12), rs.getLong(13), rs.getString(14))), rs.getString(15)));
            }
    ...
    public void add(Movie movie, String login) {
        try(Connection connection = DBUtils.getConnection()) {
            ...
            preparedStatement.setString(6, movie.getGenre() == null ? null : movie.getGenre().toString());
            preparedStatement.setString(7, movie.getMpaaRating().toString());
            preparedStatement.setString(8, movie.getScreenwriter().getName());
            preparedStatement.setString(9, movie.getScreenwriter().getBirthday() == null ? null : movie.getScreenwriter().getBirthday().toString());
    ...
    public LinkedList<Movie> getCollection() {
        LinkedList<Movie> list = new LinkedList<>(collection);
        return list;
    }
